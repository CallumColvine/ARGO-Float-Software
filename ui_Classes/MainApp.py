...
class MainApp(QMainWindow, Ui_MainApp):
    ...
    def commenceInterpolation(self):
        ...
        for Dc in xrange(julStart, julEnd, self.dayStepSize):
            ...
            for cycleJulDate in xrange(julWindowStart, julWindowEnd):
                self.checkFloatsFromIndex(floats,
                                          cycleJulDate)
            sTav = 0
            sTavSq = 0
            sTKnt = 0
    ...
    def computeDHgt(self):
        ...
        for K in range(2, self.nPress):
            ...
            self.Dh[Ipr] = self.Dh[Ipr + 1] + Q * \
                (self.Sva[Ipr + 1] + self.Sva[Ipr]) * self.stepSize
        ...
        for Ipr in range(1, self.nPress):
            ...
            if not (qSigmaT < self.Sigref_st[0]) and not (qSigmaT > self.Sigref_st[70]):
                # for Icl=1 TO 70
                #     IF qSigmaT>=Sigref_st(Icl) AND qSigmaT<=Sigref_st(Icl+1) THEN GOTO 9300
                #     NEXT Icl
                for Icl in range(0, 69):
                    if qSigmaT >= self.Sigref_st[Icl] and qSigmaT <= self.Sigref_st[Icl + 1]:
    ...
    def foundPair(self, qSigmaT, Icl):
        # found a pair
        Ra = (qSigmaT - self.Sigref_st[Icl]) / \
            (self.Sigref_st[Icl + 1] - self.Sigref_st[Icl])
        Ter = self.Sigref_te[Icl] + Ra * \
            (self.Sigref_te[Icl + 1] - self.Sigref_te[Icl])
        Sar = self.Sigref_sa[Icl] + Ra * \
            (self.Sigref_sa[Icl + 1] - self.Sigref_sa[Icl])
        Tep = qTemp - Ter
        Sap = qSal - Sar
    ...
    def getSvanom(self, S, T, P0):
        ...
        Gam = P / K35
        Pk = 1.0 - Gam
        self.Sva = self.Sva * Pk + \
            (V350p + self.Sva) * P * Dk / (K35 * (K35 + Dk))
        Svan = self.Sva * 1.0E+8
        V350p = V350p * Pk
        ...
        Dr35p = Gam / V350p
        Dvan = self.Sva / (V350p * (V350p + self.Sva))
            ...
            with open((inFileName),
                ...
                for row in reader:
                    ...
                    if (float(row[1]) > self.firstLatitude
                        ...
                        if testing:
                            floats.append("20160518_2901481.IOS")
                        else:
                            # Save it
                            floats.append(row[0])
    ...
    def prepareOutputFiles(self):
        ...
        self.hgtPath = self.outPath + "TS_Shgt.csv"     # Surface Height = Shgt
